...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ledger-net-socket-detail.ps1

node dist/tests/domainReview.test.js

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ledger-net-socket-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ledger-net-socket-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-ledger_net_socket_forge-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
